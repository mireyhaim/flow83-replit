...
export default function ExternalPaymentSuccessPage() {
  ...
  const { data: verification, isLoading, error } = useQuery({
    queryKey: ["/api/payment/external-verify", token],
    queryFn: () => 
      fetch(`/api/payment/external-verify/${token}`).then(res => res.json()),
    enabled: !!token,
    retry: 3,
    retryDelay: 1000,
  });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-pink-50 flex items-center justify-center p-4" dir="rtl">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold mb-4 text-gray-800">אישור תשלום</h1>
          <p className="text-gray-600 mb-6">
            לא נמצא קוד אישור. אם שילמת, נא לחזור לעמוד התשלום ולנסות שוב.
          </p>
